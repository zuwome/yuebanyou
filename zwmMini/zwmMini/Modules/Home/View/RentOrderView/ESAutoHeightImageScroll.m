...
- (void)resetFrameWithImage:(UIImage *)image atIndex:(NSInteger)index {
    if (index == 0) {   //获取图片后重置高度
        ...
        //frame变更，通知父视图
        !self.frameDidUpdate ? : self.frameDidUpdate(self.frame, 0);
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    UIImageView *firstImageView = self.imageViewArray[MIN(MAX(index, 0), self.imageViewArray.count - 1)];
    CGFloat firstHeight;
    ...
    UIImageView *secondImageView = self.imageViewArray[MIN(MAX(index + 1, 0), self.imageViewArray.count - 1)];
    ...
    //frame变更，通知父视图
    index = MIN(MAX(index, 0), self.imageViewArray.count - 1);
    !self.frameDidUpdate ? : self.frameDidUpdate(self.frame, index);
    //记录当前偏移量
