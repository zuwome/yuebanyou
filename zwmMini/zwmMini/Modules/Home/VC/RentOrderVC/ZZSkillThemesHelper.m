...
- (void)fetchSkillByID:(NSString *)skillID next:(requestCallback)next {
    [XJUserModel loadUser:XJUserAboutManageer.uModel.uid param:nil succeed:^(id data, XJRequestError *rError) {
        if (rError) {
            ...
            XJUserModel *user = [XJUserModel yy_modelWithDictionary:data];//[[ZZUser alloc] initWithDictionary:data error:nil];
            XJUserAboutManageer.uModel = user;
//            [[ZZUserHelper shareInstance] saveLoginer:[user toDictionary] postNotif:NO];
            
            __block XJTopic *currentTopic = nil;
            [user.rent.topics enumerateObjectsUsingBlock:^(XJTopic * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
