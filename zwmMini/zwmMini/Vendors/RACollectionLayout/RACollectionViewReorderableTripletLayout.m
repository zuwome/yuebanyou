...
@property (nonatomic, assign) RAScrollDirction scrollToDirection;
...
- (void)setUpCollectionViewGesture
{
    if (!_setUped) {
        ...
        _longPressGesture.minimumPressDuration = 0.f;
...
- (void)autoScroll
{
    ...
    if (self.scrollToDirection == RAScrollDirctionDown) {
    ...
    }else if (self.scrollToDirection == RAScrollDirctionUp) {
...
{
    [_highlightedImageView removeFromSuperview];
    switch (longPress.state) {
        case UIGestureRecognizerStateBegan: {
            _recognizerFinish = NO;
            //indexPath
            ...
            _cellFakeView.layer.shadowOpacity = .2f;
            _cellFakeView.layer.shadowRadius = 3.f;
            _cellFakeView.tag = kViewTag;
            UIImageView *cellFakeImageView = [[UIImageView alloc] initWithFrame:cell.bounds];
            _highlightedImageView = [[UIImageView alloc] initWithFrame:cell.bounds];
            cellFakeImageView.contentMode = UIViewContentModeScaleAspectFill;
            _highlightedImageView.contentMode = UIViewContentModeScaleAspectFill;
            cellFakeImageView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
            _highlightedImageView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
            cell.highlighted = YES;
            [_highlightedImageView setCellCopiedImage:cell];
            cell.highlighted = NO;
            ...
            [_cellFakeView addSubview:_highlightedImageView];
            //set center
        ...
        case UIGestureRecognizerStateChanged: {
            ...
            if (CGRectGetMaxY(_cellFakeView.frame) >= self.collectionView.contentOffset.y + (self.collectionView.bounds.size.height - _scrollTrigerEdgeInsets.bottom -_scrollTrigePadding.bottom)) {
                if (ceilf(self.collectionView.contentOffset.y) < self.collectionView.contentSize.height - self.collectionView.bounds.size.height) {
                    self.scrollToDirection = RAScrollDirctionDown;
                ...
                if (self.collectionView.contentOffset.y > -self.collectionView.contentInset.top) {
                    self.scrollToDirection = RAScrollDirctionUp;
                    [self setUpDisplayLink];
                }
            }else {
                self.scrollToDirection = RAScrollDirctionNone;
                [self invalidateDisplayLink];
            }
            break;
        }
